...
#
# The script that patches Firefox source into the Bromine source tree
# (LibreWolf privacy patches + Bromine branding/themes).
#
...
def enter_srcdir(_dir = None):
    if _dir == None:
        dir = "bromine-{}-{}".format(version, release)
    else:
        dir = _dir
    print("cd {}".format(dir))
    sys.stdout.flush()
...
def librewolf_patches():
    ...
    exec('mkdir -p lw')
    enter_srcdir('lw')
    exec('cp -v ../../settings/bromine.cfg .')
    exec('cp -v ../../settings/distribution/policies.json .')
    exec('cp -v ../../settings/defaults/pref/local-settings.js .')
    leave_srcdir();

    # Bromine modular CSS themes + loader
    exec('python3 ../scripts/install-bromine-theme.py .')
    ...
    exec("find . -path '*/appstrings.properties' -exec sed -i s/Firefox/Bromine/ {} \\;")

    # Prefer keeping LibreWolf pref-pane element IDs (paneLibrewolf) so upstream
    # patches keep applying; only soft-rebrand visible labels via locales.
    print("-> Applying Bromine / LibreWolf locales")
    l10n_dir = Path("..", "l10n")
...
if len(args) != 2:
    sys.stderr.write('error: please specify version and release of Bromine source')
    sys.exit(1)
version = args[0]
release = args[1]
srcdir = "bromine-{}-{}".format(version, release)
