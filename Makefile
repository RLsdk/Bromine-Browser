...
.PHONY : help moztree check all clean veryclean distclean patches dir bootstrap fetch build package appimage run setup-wasi check-patchfail check-fuzz fixfuzz $(docker_targets) $(woodpecker_targets) $(testing_targets)
...
lw_source_dir := bromine-$(version)-$(release)
lw_source_tarball := bromine-$(version)-$(release).source$(ext)

help:

	@echo "use: $(MAKE) [all] [check] [clean] [veryclean] [bootstrap] [build] [package] [run]"
	@echo ""
	@echo "  all         - Make Bromine source archive ${version}-${release}."
	@echo ""
	...
	@echo "                ready to build Bromine folder."
	@echo "  build       - Build Bromine (requires bootstrapped build environment)."
	@echo "  package     - Package Bromine (requires build)."
	@echo "  appimage    - Build portable x86_64 AppImage (requires package)."
	@echo "  run         - Run Bromine (requires build)."
	@echo ""
	@echo "  check-patchfail - check patches for errors."
	@echo "  check-fuzz      - check patches for fuzz."
	@echo "  fixfuzz         - fix the fuzz."
	@echo ""
	@echo ""
	...
	@echo ""
	@echo "  patches   - Just make the Bromine source directory (download, extract, patch)"
	@echo "  all       - build Bromine source tarball"
	@echo ""
	@echo "  clean     - remove all cruft except Bromine source tree"
	@echo "  veryclean - remove all except downloaded FF tarball"
	@echo "  distclean - remove all including downloads"
	@echo ""
	@echo "  moztree   - show Bromine source tree"
...
check:
	...
	@echo "Bromine release     : " $$(cat release)
	@echo ""
...
$(lw_source_dir): $(ff_source_tarball) ./version ./release scripts/librewolf-patches.py scripts/install-bromine-theme.py assets/mozconfig assets/patches.txt settings/bromine.cfg settings/distribution/policies.json
...
$(lw_source_tarball): $(lw_source_dir)
	...
	tar cf bromine-$(version)-$(release).source.tar $(lw_source_dir)
	pigz -6 bromine-$(version)-$(release).source.tar
	touch $(lw_source_dir)
...
package:
	...
	cp -v $(lw_source_dir)/obj-*/dist/bromine-$(version)-$(release).en-US.*.tar.xz .

appimage:
	@chmod +x scripts/build-appimage.sh packaging/linux/AppRun
...
build_image = bromine-build-image
